...
class ServerHandler(BaseHTTPRequestHandler):
        ...
        if(len(self.path.split('?')) > 1):
            ...
            for urlpart in self.path.split('?')[1].split("&"):
                ...
                if(len(parameter_splitet) == 2):
                    ...
                    # min 1 character
                    if(len(name_of_parameter) != 0):
                        parameter[name_of_parameter]=parameter_splitet[1]
    ...
    def send_history(self):
        ip_addr = self.client_address[0]
        response_obj = {"client_ip_address": ip_addr, "response_count": len(history[ip_addr]), "responses": history[ip_addr]}
        self.wfile.write(json.dumps(response_obj).encode('utf8'))
...
dwarf_server = ThreadedHTTPServer(('0.0.0.0',12345), ServerHandler)
try:
